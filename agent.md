...
### Creating A New Watch App Safely

Use this checklist whenever adding a new app or screen in `firmware/esp-brookesia/main/`:

1. Create a new app pair:
   - `your_app.cpp`
   - `your_app.h`
   - optional lightweight icon source such as `your_app_icon.c`
2. Register the app with `ESP_UTILS_REGISTER_PLUGIN_WITH_CONSTRUCTOR(...)`.
3. Keep the UI static-first:
   - prefer labels, lines, simple containers, and small icons
   - avoid continuous animations, pulsing effects, canvas usage, or frequent full-screen invalidation
4. Keep every custom LVGL object non-scrollable unless scrolling is explicitly needed:
   - call `lv_obj_clear_flag(obj, LV_OBJ_FLAG_SCROLLABLE)` on cards, buttons, overlays, and full-screen containers
5. Avoid large transient allocations:
   - do not build full-screen images or frame buffers in app code
   - keep text buffers bounded
   - prefer reusing labels instead of recreating widgets repeatedly
6. Treat async callbacks as unsafe by default:
   - BLE/audio/timer callbacks must guard against stale LVGL pointers
   - clear widget pointers in `close()`
   - check object validity before updating UI from asynchronous paths
7. Do not block the LVGL thread:
   - no long waits, network work, codec teardown, or BLE polling inside UI callbacks
   - offload blocking work to background tasks and only post compact UI updates back
8. If the app refreshes data, prefer push/cached state:
   - the app should display the latest known data immediately
   - live refresh should be incremental and tolerant of the phone not being ready yet

### App Layout Rules To Avoid Tearing Or Freezes

- Use a single full-screen root container with black background and no scrolling.
- Prefer 1-2 static labels updates per refresh cycle instead of recreating the whole screen.
- Avoid high-frequency LVGL timers. Use 1s or slower unless there is a strong reason.
- Avoid animation loops for status indication. Text/icon state changes are safer.
- Avoid `lv_textarea` for display-only content when a wrapped label in a plain container is enough.
- Avoid large shadows, blur-like effects, and oversized rounded layered objects.
- If an app can open/close quickly, make teardown idempotent so repeated launches cannot race resources.

### Validation Before Shipping A New App

Before considering a new app done:

1. Build with `idf.py build`.
2. Flash with `./deploy.sh`.
3. Open and close the app repeatedly on-device.
4. If the app depends on BLE/iPhone data, test:
   - app open while phone connected
   - app open while phone disconnected
   - fast re-open after close
5. Confirm:
   - no visible tearing
   - no frozen screen
   - no watchdog reset
   - no stale async callback touching freed UI
...
Always use `./deploy.sh` for flashing this project.
