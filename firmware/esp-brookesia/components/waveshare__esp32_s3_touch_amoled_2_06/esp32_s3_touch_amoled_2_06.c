...
{
    /* Keep the protected 100-row LVGL buffers, but split each flush into 10-row
     * SPI transactions. The ESP-IDF LCD transport only fires the flush callback
     * after the final chunk; the application opts these chunks into direct
     * ESP32-S3 PSRAM DMA without changing LVGL's protected buffer flags. */
    const bsp_display_config_t disp_config = {
        .max_transfer_sz = BSP_LCD_H_RES * 10 * BSP_LCD_BITS_PER_PIXEL / 8,
    };

    BSP_ERROR_CHECK_RETURN_NULL(bsp_display_new(&disp_config, &panel_handle, &io_handle));

    /* SH8601 is QSPI (panel_io), not an RGB LCD. lvgl_port_add_disp_rgb() sets the port's display type
     * to RGB so lv_disp_flush_ready() is called immediately after draw_bitmap — before the SPI DMA
     * transfer completes — which causes visible tearing on large invalidations (e.g. opening apps).
     * lvgl_port_add_disp() wires on_color_trans_done on panel_io so flush_ready tracks real TX done. */
    const int buffer_size = BSP_LCD_H_RES * LVGL_BUFFER_HEIGHT;

#if CONFIG_BSP_DISPLAY_LVGL_AVOID_TEAR || CONFIG_BSP_DISPLAY_LVGL_FULL_REFRESH || CONFIG_BSP_DISPLAY_LVGL_DIRECT_MODE
    ESP_LOGW(TAG, "LVGL avoid-tear/full/direct modes target RGB DPI panels; ignoring for SH8601 QSPI");
#endif
